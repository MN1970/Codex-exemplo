...
```
.claude/agents/
  ├── agente-saneamento.v5.0.md          # live
...
- [x] Validar 8 pilares (arquitetura doc)
- [x] Revisar R1 routing (20 agentes)
- [x] Testar R6 reranker (cross-encoder) em 100 queries
- [x] Validar R7 tiering em histórico de 1000 runs
- [x] Implementar R8 fallback (mock timeout)
- [x] Criar R9 feedback loop skeleton (Supabase schema)
- [x] Implementar R10 memory purge policy (APScheduler test)

### Phase 2 — RAG (24–48h antes go-live)

- [x] Criar 5 coleções S6–S10 em Supabase (san:v5.0:*, ene:v5.0:*, etc.)
- [x] Ingerir fontes iniciais (2000–4000 chunks/coleção)
- [x] Testar BM25 × embedding em 50 queries/coleção
- [x] Validar reranker score > 0.5 em queries críticas
- [x] Criar `rag_cache` (TTL 7 dias)
- [x] Build VERSIONS.json com checksums (MD5)
- [x] Backup RAG v4.9 → `rag_archive`

### Phase 3 — Skill versionamento (T-24h)

- [x] Copiar 5 agent skills para v5.0 (portos, aeroportos, saneamento, energia, barragens)
- [x] Copiar 4 existentes (rodovias, oae, ferrovia, metrô) → v5.0
- [x] Calcular checksums MD5 (todos 20 skills)
- [x] Criar VERSIONS.json (prefab na `.claude/agents/`)
- [x] Pin skills em settings.json local (skill_version_pin.agente-saneamento = v5.0)
- [x] Deprecate v4.9 skills (mark `deprecated_at`, 30d grace period)

### Phase 4 — Observabilidade (T-12h)

- [x] Criar tabelas Supabase: `agent_runs`, `agent_feedback`, `agent_triggers`
- [x] Setup Grafana dashboards (custo, latência, erro rate)
- [x] Configurar Slack alerts (#agent-ops) para falhas
- [x] Habilitar audit logging (imutável)
- [x] Setup APScheduler para triggers (reindex, feedback, purge)

### Phase 5 — Tiering & Fallback (T-6h)

- [x] Validar R7 complexity score em 100 queries
- [x] Testar R8 fallback (simular timeout, verify cascade)
- [x] Deploy hook PreToolUse (Haiku→Sonnet→Opus transition)
- [x] Criar fallback config per-agent (maestro.json)

### Phase 6 — Testes (T-2h)

- [x] Teste E2E: prompt em português → agente-saneamento (S8) com phase="projeto-executivo"
- [x] Teste cross-agent: "Qual o custo de tratamento de esgoto para 500k hab?" (deve usar S8 + S5 orcamento)
- [x] Teste timeout recovery (R8): força timeout em Sonnet, verifica cascade para Opus
- [x] Teste feedback loop (R9): rate 5 queries, verifica persistência em Supabase
- [x] Teste memory purge (R10): agent_memory > 100MB, verifica limpeza

### Phase 7 — Go-live (T+0)

- [x] Merge CLAUDE.md v5.0
- [x] Deploy skills v5.0 para `.claude/agents/`
- [x] Ativar RAG collections (san:v5.0:*, ene:v5.0:*, etc.)
- [x] Ativar R1 maestro routing rules (Manta 00)
- [x] Ativar R6–R10 loops (reranker, tiering, fallback, feedback, purge)
- [x] Monitorar Grafana por 1h (custo, latência, erro rate)
- [x] Anunciar time (Slack, email) com runbook de rollback
...
├── settings.json                          # Configuração v5.0 completa
