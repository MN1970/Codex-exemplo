#!/usr/bin/env python3
"""
Healthcheck for Manta Maestro v5.0
Validates checksums, RAG collections, agent skills, and routing rules.
"""
...
class HealthChecker:
    ...
    def validate_settings_json(self) -> None:
        """Validate .claude/settings.json skill pins"""
    ...
    def run(self) -> Tuple[bool, str]:
        """Run all checks"""
        print("=" * 70)
        print("Manta Maestro v5.0 Healthcheck")
        ...
        if not versions:
            ...
            return False, "VERSIONS.json loading failed"
        ...
        for msg in self.passed[:10]:  # Limit output
            print(f"  ✓ {msg}")
        ...
        if self.issues:
            ...
            status = "WARNING"
            return True, f"{len(self.warnings)} warnings"
        else:
            status = "HEALTHY"
            return True, "All checks passed"

        print(f"Status: {status}")
        return self.issues == [], f"{len(self.issues)} issues, {len(self.warnings)} warnings"


if __name__ == "__main__":
    checker = HealthChecker()
    success, message = checker.run()
    print(f"\nResult: {message}")
