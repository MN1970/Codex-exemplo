...
| **W3** Numeração & versões (D2) | Eliminar os resíduos de numeração antiga (P-02) no repo; resolver `manta_code`; marcar a versão vigente de cada documento de arquitetura e mover as antigas para histórico, no repo e no SP | 🟠 | Item a item; PR no repo |
