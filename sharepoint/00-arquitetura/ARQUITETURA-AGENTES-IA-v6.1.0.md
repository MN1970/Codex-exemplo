...
> **Nota de mirror (2026-09-16):** este arquivo é uma cópia fiel do
> documento lido em
> `Documentos Compartilhados/04_IA/Manta-Maestro/00-arquitetura/ARQUITETURA-AGENTES-IA-v6.1.0.md`
> no SharePoint (fonte autoritativa — ver `F9-meta`). O
> `ARQUITETURA-AGENTES-IA.md` (sem sufixo de versão) já versionado
> neste repositório descreve a v2.0.0/v4.2, que ficou desatualizada;
> ver nota em `CLAUDE.md` no topo do registro mestre. Os commits
> `ad98925`/`48412d1` referenciados na tabela T1/T3 acima **não foram
> encontrados no histórico deste repositório** — a reconciliação T1-T3
> parece ter sido aplicada em outro lugar (provável candidato: o repo
> espelho `manta-hub`, fora do escopo desta sessão) ou apenas
> localmente. Esta cópia é só leitura/registro; nenhuma renumeração dos
> agentes verticais existentes (`agente-portos.md` etc., ainda em
> `03-S6..S10`) foi feita nesta rodada.
